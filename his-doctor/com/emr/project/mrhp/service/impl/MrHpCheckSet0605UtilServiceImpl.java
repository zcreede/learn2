...
@Service("MrHpCheckSet0605")
public class MrHpCheckSet0605UtilServiceImpl implements MrHpCheckSet06UtilService {
   @Autowired
   ...
   public Boolean infoVerify(MrHpVo mrHpVo) throws Exception {
      ...
      if (flag) {
         MrHpDia mrHpDia = (MrHpDia)list.get(0);
         SysDiaIcd sysDiaIcd = (SysDiaIcd)dialistnew.get(0);
         if (mrHpDia.getOutcomeCd().equals("4")) {
            flag = flag && StringUtils.isNotBlank(sysDiaIcd.getDeathDiagFlag()) && sysDiaIcd.getDeathDiagFlag().equals("0");
         }
      }
